...
func New(opts Options) *fiber.App {
    ...
    app.Get("/v1/products", func(c *fiber.Ctx) error {
        ...
        if minPrice != "" { if v, err := parseMoneyToCents(minPrice); err == nil { minCents = v } }
        if maxPrice != "" { if v, err := parseMoneyToCents(maxPrice); err == nil { maxCents = v } }
        var items []Product
    ...
    app.Get("/v1/products/:id", func(c *fiber.Ctx) error {
        ...
        if err != nil { return c.Status(fiber.StatusNotFound).JSON(fiber.Map{"success": false, "message": "not found"}) }
    ...
    app.Get("/v1/categories", func(c *fiber.Ctx) error {
        rows := []struct{ Category *string `db:"category"` }{}
        ...
        for _, r := range rows { if r.Category != nil { out = append(out, *r.Category) } }
    ...
    requireAuth := srvAuth.Middleware(srvAuth.Options{CoreAPIBase: opts.CoreAPIBase})
      app.Post("/v1/shops", requireAuth, func(c *fiber.Ctx) error {
          ...
          var body struct{ Name, Slug, Description string; Meta map[string]any `json:"meta"` }
          if err := c.BodyParser(&body); err != nil { return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"success": false, "message": "invalid body"}) }
          name := strings.TrimSpace(body.Name)
          slug := strings.TrimSpace(strings.ToLower(body.Slug))
          if name == "" || slug == "" { return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"success": false, "message": "name and slug required"}) }
          var exists int
          ...
          id := uuid.New()
          if _, err := opts.DB.Exec(`INSERT INTO shops(uuid,name,slug,owner_uuid,public,description,meta) VALUES($1,$2,$3,$4,true,$5,$6)`, id, name, slug, owner, strings.TrimSpace(body.Description), body.Meta); err != nil {
              return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"success": false, "message": "db error"})
          }
    ...
    app.Post("/v1/products", requireAuth, func(c *fiber.Ctx) error {
        ...
        if shop.OwnerUUID.String() != owner { return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"success": false, "message": "not your shop"}) }
        id := uuid.New()
    ...
    app.Patch("/v1/products/:id", requireAuth, func(c *fiber.Ctx) error {
        ...
        if err := opts.DB.Get(&shopOwner, `SELECT s.owner_uuid FROM products p JOIN shops s ON s.uuid=p.shop_uuid WHERE p.uuid=$1`, id); err != nil { return c.Status(404).JSON(fiber.Map{"success": false, "message": "not found"}) }
        if shopOwner.String() != owner { return c.Status(403).JSON(fiber.Map{"success": false, "message": "forbidden"}) }
        ...
        if v, ok := body["title"].(string); ok { add("title", strings.TrimSpace(v)) }
        if v, ok := body["slug"].(string); ok { add("slug", strings.ToLower(strings.TrimSpace(v))) }
        if v, ok := body["summary"].(string); ok { add("summary", strings.TrimSpace(v)) }
        if v, ok := body["priceCents"].(float64); ok { add("price_cents", int64(v)) }
        if v, ok := body["currency"].(string); ok { add("currency", strings.ToUpper(v)) }
        if v, ok := body["stock"].(float64); ok { add("stock", int64(v)) }
        if v, ok := body["imageUrl"].(string); ok { add("image_url", strings.TrimSpace(v)) }
        if v, ok := body["published"].(bool); ok { add("published", v) }
        if v, ok := body["category"].(string); ok { add("category", strings.TrimSpace(v)) }
        if v, ok := body["images"].([]any); ok { arr := make([]string,0,len(v)); for _, e := range v { if s, ok := e.(string); ok { arr = append(arr, s) } }; add("images", pq.Array(arr)) }
        if len(sets)==0 { return c.JSON(fiber.Map{"success": true}) }
        sets = append(sets, "updated_at=now()")
        args = append(args, id)
        if _, err := opts.DB.Exec("UPDATE products SET "+strings.Join(sets, ", ")+" WHERE uuid=$"+itoa(len(args)), args...); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
    ...
    app.Delete("/v1/products/:id", requireAuth, func(c *fiber.Ctx) error {
        ...
        if err := opts.DB.Get(&shopOwner, `SELECT s.owner_uuid FROM products p JOIN shops s ON s.uuid=p.shop_uuid WHERE p.uuid=$1`, id); err != nil { return c.Status(404).JSON(fiber.Map{"success": false, "message": "not found"}) }
        if shopOwner.String() != owner { return c.Status(403).JSON(fiber.Map{"success": false, "message": "forbidden"}) }
        if _, err := opts.DB.Exec(`DELETE FROM products WHERE uuid=$1`, id); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
        return c.JSON(fiber.Map{"success": true})
    })
    ...
    app.Get("/v1/my/shops", requireAuth, func(c *fiber.Ctx) error {
        owner := srvAuth.UserID(c)
        var shops []Shop
        if err := opts.DB.Select(&shops, `SELECT uuid,name,slug,owner_uuid,public,description,created_at,updated_at FROM shops WHERE owner_uuid=$1 ORDER BY created_at DESC`, owner); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
        return c.JSON(fiber.Map{"success": true, "data": shops})
    })
    ...
    app.Post("/v1/uploads", requireAuth, func(c *fiber.Ctx) error {
        file, err := c.FormFile("file"); if err != nil { return c.Status(400).JSON(fiber.Map{"success": false, "message": "file required"}) }
        if file.Size == 0 { return c.Status(400).JSON(fiber.Map{"success": false, "message": "empty file"}) }
        ext := strings.ToLower(filepath.Ext(file.Filename)); if ext == "" { ext = ".jpg" }
        name := uuid.New().String()+ext
        dest := filepath.Join("/data/uploads/products", name)
        if err := c.SaveFile(file, dest); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "save failed"}) }
    ...
    app.Post("/v1/orders/:id/pay", requireAuth, func(c *fiber.Ctx) error {
        ...
        if err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
        if n, _ := res.RowsAffected(); n == 0 { return c.Status(400).JSON(fiber.Map{"success": false, "message": "not payable"}) }
        return c.JSON(fiber.Map{"success": true})
    })
    ...
    app.Post("/v1/dev/seed", requireAuth, func(c *fiber.Ctx) error {
        ...
        if count == 0 {
            shopID = uuid.New()
            if _, err := opts.DB.Exec(`INSERT INTO shops(uuid,name,slug,owner_uuid,public,description) VALUES($1,'Demo Shop','demo-shop',$2,true,'Sample demo shop')`, shopID, owner); err != nil { return c.Status(500).JSON(fiber.Map{"success": false}) }
        } else {
    ...
    app.Get("/v1/search", requireAuth, func(c *fiber.Ctx) error {
        q := strings.TrimSpace(c.Query("q"))
        if len(q) < 2 { return c.JSON(fiber.Map{"success": true, "data": fiber.Map{
...
func ensureCart(db *sqlx.DB, user string) (uuid.UUID, error) {
    ...
    if err == sql.ErrNoRows {
        id = uuid.New()
        if _, e := db.Exec(`INSERT INTO carts(uuid,user_uuid) VALUES($1,$2)`, id, user); e != nil { return uuid.Nil, e }
        return id, nil
    }
    return id, err
}
...
func getCart(c *fiber.Ctx, db *sqlx.DB) error {
    ...
    if err := db.Select(&items, q, cartID); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
    var total int64
    currency := "USD"
    for _, it := range items { total += int64(it.Quantity) * it.PriceCents; currency = it.Currency }
...
func addCartItem(c *fiber.Ctx, db *sqlx.DB) error {
    user := srvAuth.UserID(c)
    var body struct{ ProductID string `json:"productId"`; Quantity int `json:"quantity"` }
    if err := c.BodyParser(&body); err != nil || body.ProductID == "" || body.Quantity <= 0 { return c.Status(400).JSON(fiber.Map{"success": false, "message": "invalid body"}) }
    cartID, err := ensureCart(db, user)
    if err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
    // upsert
...
func updateCartItem(c *fiber.Ctx, db *sqlx.DB) error {
    ...
    if err := c.BodyParser(&body); err != nil { return c.Status(400).JSON(fiber.Map{"success": false, "message": "invalid body"}) }
    if body.Quantity <= 0 {
        if _, err := db.Exec(`DELETE FROM cart_items WHERE uuid=$1`, id); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
        return c.JSON(fiber.Map{"success": true})
    }
    if _, err := db.Exec(`UPDATE cart_items SET quantity=$1 WHERE uuid=$2`, body.Quantity, id); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
    return c.JSON(fiber.Map{"success": true})
}
...
func clearCart(c *fiber.Ctx, db *sqlx.DB) error {
    ...
    cartID, err := ensureCart(db, user)
    if err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
    if _, err := db.Exec(`DELETE FROM cart_items WHERE cart_uuid=$1`, cartID); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
    return c.JSON(fiber.Map{"success": true})
}
...
func createOrderFromCart(c *fiber.Ctx, db *sqlx.DB) error {
    ...
    if err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
    // load cart
    type row struct{ ProductUUID uuid.UUID `db:"product_uuid"`; Quantity int `db:"quantity"`; PriceCents int64 `db:"price_cents"`; Currency string `db:"currency"` }
    var rows []row
    if err := db.Select(&rows, `SELECT ci.product_uuid, ci.quantity, p.price_cents, p.currency FROM cart_items ci JOIN products p ON p.uuid=ci.product_uuid WHERE ci.cart_uuid=$1`, cartID); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
    if len(rows) == 0 { return c.Status(400).JSON(fiber.Map{"success": false, "message": "cart empty"}) }
    var total int64; currency := rows[0].Currency
    for _, r := range rows { total += int64(r.Quantity) * r.PriceCents }
    orderID := uuid.New()
    tx, err := db.Beginx(); if err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
    defer tx.Rollback()
    if _, err := tx.Exec(`INSERT INTO orders(uuid,user_uuid,total_cents,currency,status) VALUES($1,$2,$3,$4,'pending')`, orderID, user, total, currency); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
    for _, r := range rows {
        if _, err := tx.Exec(`INSERT INTO order_items(order_uuid,product_uuid,quantity,price_cents) VALUES($1,$2,$3,$4)`, orderID, r.ProductUUID, r.Quantity, r.PriceCents); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
    }
    if _, err := tx.Exec(`DELETE FROM cart_items WHERE cart_uuid=$1`, cartID); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
    if err := tx.Commit(); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
...
func listOrders(c *fiber.Ctx, db *sqlx.DB) error {
    user := srvAuth.UserID(c)
    var out []Order
    if err := db.Select(&out, `SELECT uuid,total_cents,currency,status,created_at FROM orders WHERE user_uuid=$1 ORDER BY created_at DESC`, user); err != nil { return c.Status(500).JSON(fiber.Map{"success": false, "message": "db error"}) }
    return c.JSON(fiber.Map{"success": true, "data": out})
}
...
func getOrder(c *fiber.Ctx, db *sqlx.DB) error {
    ...
    id := c.Params("id")
    var o Order
    if err := db.Get(&o, `SELECT uuid,total_cents,currency,status,created_at FROM orders WHERE uuid=$1 AND user_uuid=$2`, id, user); err != nil { return c.Status(404).JSON(fiber.Map{"success": false, "message": "not found"}) }
